package com.example.common.path;
...
public enum PyPath {
        ...
        try {
            ...
            if (resource.exists()) {
                ...
                if (file.exists() && file.isFile()) {
                    log.info("找到Python脚本文件: {}", file.getAbsolutePath());
                    return file.getAbsolutePath();
                }
            }

            // 3. 如果无法直接访问文件（打包后环境），提取到临时文件
            log.info("从JAR包中提取Python脚本: {}", classPath);
            return extractFromJar(resource);

        } catch (Exception e) {
            log.error("无法加载Python脚本: {}", classPath, e);
            throw new RuntimeException("无法加载Python脚本: " + classPath, e);
        }
    }
    ...
    private String extractFromJar(Resource resource) throws IOException {
        ...
        log.info("Python脚本已提取到临时文件: {}", tempFile.getAbsolutePath());
